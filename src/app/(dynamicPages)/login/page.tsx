...
const Login = () => {
    ...
    const signIn = async () => {
        try {
            const result = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URI}auth/login/`, user);
            ...
            if (error?.status === 500) {
                setErrorMsg('Oops, Please try again in a few minutes.')
            };
        }
    }
    ...
    return (
        <form className={`${styles.form_container} relative top-10`}>
            ...
            <div className={styles.input_container}>
                ...
                <input placeholder="name@mail.com" title="Input title" name="input-name" type="text"
                    className={styles.input_field} id="email_field" onChange={(e) => { setUSer({ ...user, account: e.target.value }) }} />
            </div>
            <div className={styles.input_container}>
                ...
                <input placeholder="Password" title="Input title" name="input-name" type="password"
                    className={styles.input_field} id="password_field" onChange={(e) => { setUSer({ ...user, password: e.target.value }) }} />
            </div>
            <span className='text-red-500'>{errorMsg}</span>
            <button title="Sign In" type="submit" className={styles.sign_in_btn} onClick={(e) => { e.preventDefault(); signIn() }}>
                <span>Sign In</span>
            </button>
